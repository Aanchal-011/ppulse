...
# 0: Not Stressed, 1: Stressed, 2: Overly Stressed
y_category = pd.cut(y_stress_level, bins=[-1, 1, 3, 4], labels=[0, 1, 2])

# Now, split the data into training and testing sets
X_train, X_test, y_train, y_test = train_test_split(
    X, y_stress_level, test_size=0.2, random_state=42)
...
def predict():
    try:
        ...
        print(data)
        features = np.array([data['sr'], data['rr'], data['t'], data['lm'],
                            data['bo'], data['rem'], data['sh'], data['hr']]).reshape(1, -1)
